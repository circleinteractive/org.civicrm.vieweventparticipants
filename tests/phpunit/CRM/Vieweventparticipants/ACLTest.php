...
class CRM_Vieweventparticipants_ACLTest extends CRM_Vieweventparticipants_BaseTestClass {
  ...
  private $basicPermissions = ['access CiviCRM', 'add contacts', 'view my contact', 'edit my contact', 'access CiviEvent', 'view event info', 'register for events'];
  ...
  public function testNoParticipants() {
    ...
    CRM_Core_Config::singleton()->userPermissionClass->permissions = array_merge($this->basicPermissions, ['view my event participants']);
    ...
    CRM_Core_Config::singleton()->userPermissionClass->permissions = array_merge($this->basicPermissions, ['edit my event participants']);
  ...
  protected function createContactsAndEvents() {
    ...
    $ada_id    = $this->individualCreate(['first_name' => 'Ada', 'last_name' => 'Ant']);
    $bert_id    = $this->individualCreate(['first_name' => 'Bert', 'last_name' => 'Bee']);
    $celia_id = $this->individualCreate(['first_name' => 'Celia', 'last_name' => 'Caterpillar']);
    $derek_id = $this->individualCreate(['first_name' => 'Derek', 'last_name' => 'Dung-Beetle']);
    $elsie_id = $this->individualCreate(['first_name' => 'Elsie', 'last_name' => 'Earwig']);

    $this->contactIds = [$user_id, $ada_id, $bert_id, $celia_id, $derek_id, $elsie_id];
  ...
  protected function createGroupACL() {
    // Create ACL group.
    $aclGroup = $this->callAPISuccess('Group', 'create', [
      'title' => 'Test ACL group',
      'name' => 'Test_ACL_group',
      'is_active' => 1,
      'group_type' => ['1' => 1], // 1 = ACL group
    ]);
    ...
    $targetGroup = $this->callAPISuccess('Group', 'create', [
